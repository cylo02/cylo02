# Hello, I'm Cyril!👋

**`Financial Management Graduate | Aspiring Data Analyst | Interested in Data Analytics`**

Welcome to my GitHub! I'm an enthusiastic problem-solver with a keen interest in data analysis, visualization, and storytelling. My goal is to leverage data-driven solutions to make impactful decisions in today's fast-paced world. I graduated with Bachelor of Science in Business Administration, Major in Financial Management in Polytechnic University of the Philippines - Manila. 

[![Facebook](https://img.shields.io/badge/Facebook-%231877F2.svg?logo=Facebook&logoColor=white)](https://facebook.com/https://facebook.com/cyril.lorenzoo/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/https://www.linkedin.com/in/cylorenzo/) 

---

## Languages and Tools
<img align="left" alt="MySQL" width="30px" style="padding-right:10px;"  src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" />
<img align="left" alt="Power BI" width="30px" style="padding-right:10px;"  src="https://i.postimg.cc/rps4JpBx/icons8-power-bi-logo-48.png" />
<img align="left" alt="MS Excel" width="30px" style="padding-right:10px;"  src="https://i.postimg.cc/vTVTgwwR/Microsoft-Office-Excel-Logo.png" />
<br />

#

## Projects

<h3>Project 1: Sales Report</h3>
<p>This project involves designing a comprehensive Power BI report to analyze and visualize the performance of Adventure Works, a fictional company from Microsoft's official sample dataset. The goal was to provide actionable insights into key business metrics while showcasing advanced data visualization skills.</p>
<p><b> Skills Used:</b> MS Excel, Power BI, Data Cleaning and Transformation, Data Visualization</p>

<img align="left" alt="" width="2767px" style="padding-right:10px;"  src="https://i.postimg.cc/L8DK9c52/Adventure-Works-Final-2.png" />


<h3>Project 2: Sales Dashboard</h3>
<p>The dataset used was taken from a free training provider, Data Analytics Philippines. The goal was to provide a summary of key metrics that is useful to provide business insights.</p>
<p><b> Skills Used:</b> MS Excel, Power BI, Data Cleaning and Transformation, Data Visualization</p>

<img align="left" alt="" width="2767px" style="padding-right:10px;"  src="https://i.postimg.cc/13hwmdTC/project-1.png" />

<h3>Project 3: Sales Retail Analysis</h3>
<p>This project involves Exploratory Data Analysis (EDA) on the dataset taken from Kaggle. The objective was to uncover meaningful insights into the company's sales, customers, and product performance through data exploration techniques.
</p>
<p><b> Skills Used:</b> MS Excel, Data Cleaning and Transformation, Data Visualization</p>

<img align="left" alt="" width="2767px" style="padding-right:10px;"  src="https://i.postimg.cc/sgGzc6gr/Retail-Sales-Analysis-1.png" />
## Skills

<p>• MS Excel (VLookup, Pivot Tables, Power Query)<p/>
<p>• MS Power BI (Data Modeling, Data Visualization)<p/>
<p>• SQL Database (MySQL)<p/>
<p>• Other MS Office Suite Applications<p/>
<p>• Analytical Thinking<p/>
<p>• Team Player<p/>
<p>• Communication Skills (Written and Verbal)<p/>
<p>• Adaptability and Flexibility<p/>

#


  ![snake gif](https://github.com/cylo02/cylo02/blob/output/github-snake.svg)
